...
img = Image.open(r'forest.jpg')
img_style = Image.open(r'v.png')
...
# ПРЕДОБРАБОТКА ФОТО С RBG НА BGR
...
# ФУНКЦИЯ КОТОРАЯ ПРЕОБРАЗОВЫВАЕТ ИЗОБРАЖЕНИЯ ИЗ ФОРМАТА BGR в RGB
def deprocess_img(procesed_img):
    x = procesed_img.copy()
    if len(x.shape) == 4:
        x = np.squeeze(x, 0)
...
# ЗАГРУЖАЕМ СВЕРТОЧНУЮ СЕТЬ БЕЗ ПОЛНОСВЯЗНОЙ НА КОНЦЕ 
